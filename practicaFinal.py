...
valoracionesCSV = spark.read.csv('valoraciones_EP.csv', header=False, inferSchema=True, encoding="UTF-8").toDF(
...
ratingsALS = ratingsTotal.withColumn("rating", col("rating").cast("double")).withColumnRenamed("user_id",
...
animeCSV = animeCSV.join(columnaValoracionMedia, animeCSV.ID == columnaValoracionMedia.itemId, how="left").drop(
    "itemId")
...
genreRatingsScore = animeGenres.groupBy("genre").agg(avg("Score").alias("media_score"),
                                                     count("*").alias("Numero de animes")).orderBy(
    col("media_score").desc())
print("Por Score")
...
df_mediaScore = animeGenres.groupBy("genre").agg(avg("valoracion_media").alias("media_valoracion")).orderBy(
...
studioRatingsScore = studioRatings.groupBy("studio").agg(avg("Score").alias("media_score"),
                                                         count("*").alias("Numero de animes")).orderBy(
    col("media_score").desc())
studioRatingsValoracion = studioRatings.groupBy("studio").agg(avg("valoracion_media").alias("media_valoracion"),
                                                              count("*").alias("Numero de animes")).orderBy(
    col("media_valoracion").desc())
# mejor valoracion
...
df_usuario_EP = ratingsEP.join(animeGenres, ratingsEP.itemId == animeCSV.ID, "left").groupBy("genre").agg(
...
generos_EP = ratingsEP.join(animeGenres, ratingsEP.itemId == animeGenres.ID, "left").groupBy("genre").agg(
    avg("rating").alias("media_usuario")).toPandas()
...
df_porcentaje = animeCSV.groupBy("Type").agg(sum("Dropped").alias("total_dropped"),
                                             sum("Completed").alias("total_completed")).toPandas()

# Calcular porcentajes
df_porcentaje["pct_completed"] = df_porcentaje["total_completed"] / (
            df_porcentaje["total_completed"] + df_porcentaje["total_dropped"])
df_porcentaje["pct_dropped"] = df_porcentaje["total_dropped"] / (
            df_porcentaje["total_completed"] + df_porcentaje["total_dropped"])
...
recs_final = recs.select(explode(col("recommendations")).alias("rec")).select(col("rec.itemId").alias("anime_id"),
                                                                              col("rec.rating").alias(
                                                                                  "predicted_rating")).join(animeCSV,
                                                                                                            col("anime_id") == animeCSV.ID,
                                                                                                            "inner").select(
...
for tipo in ["movie", "tv"]:
    ...
    (
        ...
        .limit(5)
        .select(concat_ws(" | ", col("anime_id"), col("titulo_original"), col("titulo_ingles"),
                          col("valoracion_media")).alias("value"))
        .coalesce(1)
        .write
        .mode("overwrite")
        .text(ruta_tipo)
